...
`assign`用于连续赋值，该语句可综合成组合逻辑电路，该语句的赋值对象必须是`wire`
...
运算符基本同c语言相同，这里只讲拼接与重复，因为这两个还是需要用的

拼接运算符使用`{...}` ，拼接的操作数必须是有确定位宽的
...
拼接后的操作数也可以出现在赋值运算的左侧，只要满足要求即可（过程赋值左侧必须是保值的类型）
...
`always`循环执行，每个`always`语句均从仿真时刻0开始执行，并一直循环执行

过程赋值语句（包括initial和always）的赋值对象必须是保值的，即不能为`wire`

例如赋值左侧必须是`reg` ，`integer` ，`real`等，或者是他们的某些位，或者是他们的拼接

同一个对象只能在一个`always`中被赋值，即只能在一个`always`中的赋值运算符左侧
...
`case`语句可以一次匹配多个不同的常量来执行相同的语句，只需要使用`,`隔开即可，例如

```verilog
x1,x2,x3:statement;
```

这样当表达式的值等于x1或x2或x3时均执行statement语句
