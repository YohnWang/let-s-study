...
需要注意的是，嵌套函数只能在该函数的作用域内使用，当这个函数返回后，如果其他地方调用了它的函数指针将会引发错误
...
`__attribute__((cleanup(clean_function)))` 

该属性会给变量绑定一个函数，当这个变量离开它的作用域后会执行这个函数，这个函数的返回值将会被忽略，与c++的RAII类似，它表现地像析构函数，因此不允许抛出异常
...
int foo()
{
	int __attribute__((cleanup(del))) *x=malloc(sizeof(*x));
    ...
    return 0
}
```
