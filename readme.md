...
这是编写程序的必要准备 编写的程序过程中 我们需要编辑器 其中 你可以选择一款自己喜欢的编辑器 比如 Sublime 或者NotePad++ 以及其他的编辑器
...
一个程序至少能计算 而计算就需要数 c语言提供了一些数据类型用于表示数据
...
字符数组使用`'\0'`表示字符数组结束  所以存储空间会占用掉一个
...
str是一个字符数组 但是由于初始化和赋值的区别 上述代码是错误的 
